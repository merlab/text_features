# purpose: for extracting data from PDXE datasets
library(PharmacoGx)
...
#'
#' @export
summarizeDataPGx <- function(pSet, mDataType,
                             drug,
  ...
  exp <- PharmacoGx::summarizeMolecularProfiles(pSet, mDataType = mDataType, ...)
  clid <- exp$cellid
  for (s in asm)
  {
