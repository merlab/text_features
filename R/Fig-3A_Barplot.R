...
df <- data.frame(method=c("Text-Mining", "Auto-HMM-LMF", "STF", "SIRS", "ISIRS", "ENR"),
...
p<-ggplot(data=df, aes(x=method, y=cor, fill=method),  alpha = .15) +
  geom_bar(stat="identity") + coord_flip() + scale_fill_manual(values=rep(clr[2],6))
p <- p + theme(legend.position="none", axis.title.x = element_text(size = 16), axis.title.y = element_text(size = 16),
