set.seed(7)
library(ggplot2); library(ggridges); library(ggpubr)
library(rstatix)
...
for(dr in unique(fdf$drug))
{
  ...
  dfl[[dr]] <- data.frame(Correlation= c(tm, ntm), Type=c(rep('Text', length(tm)),
...
createDF <- function(dfl, drugOrd=NA)
{
  ...
  if(is.na(drugOrd)){
  mdiff <- sapply(dfl, function(i){ mean(i$Correlation[i$Type=='Text']) - mean(i$Correlation[i$Type=='Other'])})
  ...
  mdf <- data.frame(t(sapply(levels(df$drug), function(d){
    c('Text'  = mean(df[df$drug==d & df$Type=='Text', 'Correlation']),
...
dfall <- dfallx$df; mdx <- dfallx$mean; mdx$drug <- rownames(mdx)
...
plt <- plt + stat_pvalue_manual(stat.test, label = "ptxt", size = 2.75,
                                tip.length =0.01, fontface = 'italic') +
  scale_y_continuous(expand = expansion(mult = c(0.02, 0.04)))

txtCol <- setNames(c( #"#404040", "#ca0020") #"#374E55FF", "#B24745FF"
                       "#CD534CFF", "#868686FF")
                   , c('Text mining','Other'))
...
fillCol <-  setNames(rep_len(bgCol,  length.out = length(levels(df$drug))),
                     levels(df$drug))
drn <- 1:length(fillCol)

pltDr <- ggplot(df) + geom_rect(data = df[drn, ], xmin = drn - 0.5, xmax = drn + 0.5,
            ymin = -Inf, ymax = Inf, fill = fillCol, alpha = 0.2)
pltDr <- pltDr + geom_violin(aes(x = drug, y = Correlation, fill = Type), position = dodge,
                             alpha=0.65, size=0.5,color='#f0f0f0', trim=T)
pltDr <- pltDr + geom_boxplot(aes(x = drug, y = Correlation, fill = Type),position = dodge,
...
pltDr <- pltDr + stat_pvalue_manual(stat.test, label = "ptxt", size = 2.0,
