...
plot_univariate <- function(pSet, drugname){
  tm <- readRDS(sprintf("../train_output/univariate/%s_%s_tm.rds", pSet,drugname))
  ntm <- readRDS(sprintf("../train_output/univariate/%s_%s_ntm.rds", pSet,drugname))
  data <- data.frame(
  ...
  plt <- plt + theme(legend.position="none") + labs(title=sprintf("%s COR", drugname),x="Feature Selection", y = "COR")
  return(plt)
}
